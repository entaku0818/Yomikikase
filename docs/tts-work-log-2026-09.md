...
### 2. ~~`g` → `ɡ` のバグ修正~~ / ~~読み下しの追加~~（2026-09-19 対応済み）
`JapaneseG2PProcessor.swift` をまとめて直した。詳細は下の
「[日本語 G2P の読み下しを直した](#日本語-g2p-の読み下しを直した2026-09-19)」。
**実機で鳴るかの確認（上の 1）はこの修正後の音でやること。**

### 3. クラウドTTSの後始末（新バージョン公開後）
...
## 日本語 G2P の読み下しを直した（2026-09-19）

`JapaneseG2PProcessor.swift`（実体は `VoiceYourText/Features/KokoroTTS/KokoroSwiftSources/`、
`LocalPackages/KokoroSwift/` にミラー）を修正。回帰テストは
`VoiceYourTextTests/JapaneseG2PProcessorTests.swift`（24ケース）。

### 直したもの

| | 症状 | 原因 | 対応 |
|---|---|---|---|
| **/g/ が全消え** | 神奈川→カナアワ、助詞「が」→「あ」 | 変換表が ASCII `g`(U+0067) を出すが Kokoro の語彙は IPA `ɡ`(U+0261) だけ | 変換表の が行・ぎゃ行を `ɡ` に |
| **句読点が全消え** | 文の区切りで一切ポーズが入らない | CFStringTokenizer は `、`/`。`/`「` を**半角形** `､`/`｡`/`｢`(U+FF6x) で返す。変換表は全角しか持っていないので全部落ちていた | 読み正規化に NFKC を入れて全角に畳む。`『』【】〈〉・‥` も表に追加 |
| **数字が壊れる** | `0.5秒`→「5びょう」、`1,400円`→「400えん」と**桁が消える**。`1人`→`1rén` とピンインが混入 | CFStringTokenizer が数字混じりの日本語で誤動作 | **トークナイザに渡す前**に数字をかなへ開く（`prepareForTokenization`）。副作用で `4月`→よんがつ、`19日`→じゅうきゅうにち と助数詞の読みも改善（数字のままだと `4つき`/`19か`） |
| **英字が消える** | `iOS` が無音 | 変換表に無い文字は捨てられる | 略語（4文字以下 or 全大文字8文字以下）は1文字ずつアルファベット読み（`iOS`→あいおーえす）、語らしい並びは ICU の Latin→Hiragana に流す。`Google`/`PDF` はトークナイザが元々読んでくれる |
| **「おう」が /o/+/u/** | 「とうきょう」が「とうきょうう」寄り | `oɯ` をそのまま渡していた | `oɯ`→`oː` に畳む |
| 半角カタカナ・`ヴァ`・小書きかな | 無音 | 表に無い | NFKC＋カタカナ→ひらがな正規化、`ゔぁ〜ゔぉ`・`しぇ/ちぇ/じぇ/つぁ`・小書きかな単独を表に追加 |

### 判断したこと

- **同一母音の連続（`aa`/`ii`/`ɯɯ`/`ee`/`oo`）は畳まない。** Kokoro 側で母音2つ＝伸びた母音として
  鳴るので畳む必要がなく、畳むと `県に行く`(`ɲii`)のような形態素境界をまたぐ並びを壊す。
  必要だったのは `oɯ`→`oː` だけ。
- **数字はトークナイザの前で開く。** 後ろでやると上表のとおり桁が消えた後になる。
  かなに開いた方が助数詞の読みも良くなるので一石二鳥（ただし `5分`→ごぶん のように
  `ごふん` から悪化するケースもある。トレードオフとして受容）。
- **回帰の当て方**: Kokoro の語彙にない文字は `Tokenizer.tokenize` が無言で捨てるので、
  「語彙外の文字を出さない」を不変条件としてテストにした。語彙は app bundle の
  `config.json` から読む（読めない環境では XCTSkip）。加えて ASCII `g` は語彙に**唯一**
  存在しない ASCII 英字なので、バンドル無しでも `g` の混入だけは常に検出できる。

### まだ残っている読みの誤り（既知・トークナイザ由来）

`お母さん`→おははさん / `私`→わたくし / `3日`→さんにち（みっか）/ `5分`→ごぶん（ごふん）/
`1兆`→いちちょう（いっちょう）。いずれも CFStringTokenizer の読み付けの限界で、
辞書を持たない限り直せない。音としては鳴るので優先度は低い。
